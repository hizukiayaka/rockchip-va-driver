...
static int rk_vp8_encoder_after_encode(struct rk_venc *enc,
				       uint32_t outputStreamSize);
static int rk_vp8_encoder_before_encode(struct rk_venc *enc);
static int rk_vp8_encoder_init(struct rk_venc *enc,
...
static void rk_vp8_encoder_setconfig(struct rk_venc *enc,
...
static void rk_vp8_encoder_store_priv_data(struct rk_vp8_encoder *enc,
					   ...
					   int32_t type);
static int rk_vp8_encoder_updatepriv(struct rk_venc *enc,
			       ...
			       struct rk_vepu_init_param *enc_parms)
{
	...
	EncoderParameters *cml = &ienc->cmdl;
	...
	if (OpenEncoder(cml, &ienc->encoder) != 0)
		return -1;

	/* First frame is always intra with zero time increment */
	ienc->encIn.codingType = VP8ENC_INTRA_FRAME;
	ienc->encIn.timeIncrement = 0;

	ienc->priv_data = (uint8_t *) calloc(1, 5487);
	if (ienc->priv_data == NULL) {
		VPU_PLG_ERR("allocate private data buffer failed\n");
		CloseEncoder(ienc->encoder);
		return -1;
	}
				     ...
				     struct rk_vepu_runtime_param *param)
{
	...
	VP8EncGetRateCtrl(ienc->encoder, &rc);
	...
	if (param->framerate_denom != 0 && param->framerate_numer != 0 &&
		...
		ienc->cmdl.outputRateNumer = param->framerate_numer;
		ienc->cmdl.outputRateDenom = param->framerate_denom;
		reset = true;
	}
	...
	if (reset) {
		...
		ienc->cmdl.streamSize = 0;
		ienc->cmdl.frameCntTotal = 0;
		ret = VP8EncSetRateCtrl(ienc->encoder, &rc);
		if (ret < 0) {
	...
	if (param->keyframe_request) {
		ienc->cmdl.keyframeRequest = true;
	}
}
				       ...
				       uint32_t outputStreamSize)
{
	...
	ret =
	    VP8EncStrmEncodeResult(ienc->encoder, &cml->encOut,
				   outputStreamSize);

	VP8EncGetRateCtrl(ienc->encoder, &cml->rc);

	cml->streamSize += cml->encOut.frameSize;

	MaAddFrame(&cml->ma, cml->encOut.frameSize * 8);

	ienc->encIn.timeIncrement = cml->outputRateDenom;
	...
	if ((cml->frameCntTotal + 1) && cml->outputRateDenom) {
		...
		cml->bitrate =
		    (uint32_t) (8 *
				(tmp / (uint32_t) cml->outputRateDenom));
	}
	...
	VPU_PLG_INF
	    ("Total of %llu frames processed, %d frames encoded, %d bytes.\n",
	     cml->frameCntTotal, ienc->codedFrameCnt, cml->streamSize);
...
static int rk_vp8_encoder_before_encode(struct rk_venc *enc)
{
	...
	if (cml->outputRateDenom)
		cml->ma.length =
		    MAX(1,
			MIN(cml->outputRateNumer / cml->outputRateDenom,
	...
	if (cml->inputFormat <= 1) {
		ienc->src_img_size = cml->lumWidthSrc * cml->lumHeightSrc +
	...
	if (VP8EncGetRateCtrl(ienc->encoder, &cml->rc) < 0) {
	...
	if (cml->keyframeRequest ||
	    (cml->intraPicRate != 0 &&
	     ienc->intraPeriodCnt >= cml->intraPicRate)) {
		ienc->encIn.codingType = VP8ENC_INTRA_FRAME;
		ienc->intraPeriodCnt = 0;
		cml->keyframeRequest = false;
	} else {
		ienc->encIn.codingType = VP8ENC_PREDICTED_FRAME;
	}

	/* This applies for PREDICTED frames only. By default always predict
	 * from the previous frame only. */
	ienc->encIn.ipf = VP8ENC_REFERENCE_AND_REFRESH;
	ienc->encIn.grf = ienc->encIn.arf = VP8ENC_REFERENCE;

	/* Encode one frame */
	ret =
	    VP8EncStrmEncode(ienc->encoder, &ienc->encIn, &cml->encOut, cml);
	if (ret < 0) {
		VPU_PLG_ERR("Generate Encoder configuration failed\n");
		return -1;
	}

	VP8EncGetFrameHeader(ienc->encoder,
			     (uint8_t **) & ienc->rk_payloads[0],
			     &ienc->rk_payload_sizes[0]);
	VP8EncGetRegs(ienc->encoder, (uint32_t **) & ienc->rk_payloads[1],
		      &ienc->rk_payload_sizes[1]);
	rk_vp8_encoder_getpriv(ienc, (uint8_t **) & ienc->rk_payloads[2],
			       &ienc->rk_payload_sizes[2]);
	return 0;
}
					   ...
					   int32_t type)
{
	...
	uint8_t *data =
	    (uint8_t *) enc->priv_data + PRIVATE_DATA_HEADER_SIZE;
				     ...
				     void *config, uint32_t cfglen)
{
	...
	return VP8EncSetProbCnt(ienc->encoder, (uint8_t *) config, cfglen);
}

static void rk_vp8_encoder_deinit(struct rk_venc *enc)
{
	struct rk_vp8_encoder *ienc = (struct rk_vp8_encoder *) enc;
...
static int OpenEncoder(EncoderParameters * cml, VP8EncInst * pEnc)
{
	...
	VPU_PLG_DBG("Init config: size %dx%d   %d/%d fps  %d refFrames\n",
		    ...
		    cfg.frameRateDenom, cfg.refFrameAmount);
	...
	VPU_PLG_INF("Get rate control: qp=%2d [%2d..%2d] %8d bps,"
		    ...
		    rcCfg.bitPerSecond, rcCfg.pictureRc,
		    rcCfg.bitrateWindow);
	...
	if (codingCfg.roi2Area.enable)
		VPU_PLG_DBG("  ROI 2: %d  %dx%d-%dx%d\n",
			    codingCfg.roi2DeltaQp, codingCfg.roi2Area.left,
...
void PrintFrame(EncoderParameters * cml, VP8EncInst encoder,
		uint32_t frameNumber, VP8EncRet ret)
{
	if ((cml->frameCntTotal + 1) && cml->outputRateDenom) {
		/* Using 64-bits to avoid overflow */
		uint64_t tmp = cml->streamSize / (cml->frameCntTotal + 1);
	...
	VPU_PLG_DBG("%s",
		    ...
		    "lost" : (cml->encOut.codingType ==
			      VP8ENC_INTRA_FRAME) ? " I  " : (cml->encOut.
							      codingType ==
							      VP8ENC_PREDICTED_FRAME)
		    ? " P  " : "skip");
	...
	if (cml->encOut.frameSize != (cml->encOut.streamSize[0] +
				      ...
				      cml->encOut.streamSize[2])) {
		VPU_PLG_DBG
		    ("ERROR: Frame size doesn't match partition sizes!\n");
	}
}
